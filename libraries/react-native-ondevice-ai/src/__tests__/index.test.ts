...
describe("react-native-ondevice-ai", () => {
  ...
  describe("chatStream", () => {
    ...
    it("serializes concurrent streams before registering the next listener", async () => {
      ...
      const initialCalls = mock.chatStream.mock.calls.length;
      ...
      expect(mock.chatStream).toHaveBeenCalledTimes(initialCalls + 1);
      resolveFirst({
        message: "first",
        conversationId: "",
        canContinue: true,
      ...
      await first;
      await Promise.resolve();
      expect(mock.chatStream).toHaveBeenCalledTimes(initialCalls + 2);
      await second;
    });
  });
  ...
  describe("translateStreaming", () => {
    it("should return translate result", async () => {
      const result = await translateStreaming("Hello", {
        targetLanguage: "ko",
      });
      expect(result).toHaveProperty("translatedText");
    });
    ...
    it("should invoke onChunk with final chunk", async () => {
      const chunks: TextStreamChunk[] = [];
      await translateStreaming("Hello", {
        targetLanguage: "ko",
