...
import {summarize} from 'react-native-ondevice-ai';
...
export function GuardrailDemo() {
  const [input, setInput] = useState(
    'Artificial intelligence is transforming how we build software. On-device AI keeps data private.',
  );
  ...
  const patterns = blockedPatterns
    ...
    .filter(Boolean);
  const hasBlockedContent = patterns.some((p) => input.toLowerCase().includes(p));
  ...
  const handleRun = async () => {
    ...
    if (isOverLimit) {
      setResult({
        type: 'blocked',
        message: `Blocked by InputLengthGuardrail: Text is ${input.length} characters (max: ${maxLength})`,
      });
    ...
    if (hasBlockedContent) {
      ...
      setResult({
        type: 'blocked',
        message: `Blocked by ContentFilterGuardrail: Contains prohibited pattern "${found}"`,
      });
      setProcessingTime(Date.now() - start);
      setIsProcessing(false);
      return;
    ...
    try {
      ...
      const sumResult = await summarize(input);
      console.log('[DEBUG] guardrail summarize response:', JSON.stringify(sumResult));
      setResult({
        type: 'success',
        message: sumResult.summary,
      });
      setProcessingTime(Date.now() - start);
    } catch (e: any) {
      setResult({type: 'blocked', message: `Error: ${e.message}`});
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <CodePatternCard
        ...
        code={`// Swift - Guardrail-wrapped chain
...
let result = try await chain.run(text)`}
      ...
      <TextInput
        ...
        onChangeText={setBlockedPatterns}
        placeholder="e.g. password, SSN, credit card"
      ...
      <TouchableOpacity
        ...
        {isProcessing ? (
          ...
          <Text style={styles.runButtonText}>Run with Guardrails</Text>
        )}
      </TouchableOpacity>
      ...
      {result && (
        <View
          style={[
            styles.resultCard,
            result.type === 'blocked' && styles.resultBlocked,
          ]}>
          {processingTime !== null && (
            <View style={styles.badgeRow}>
              ...
              <StatBadge
                label="Status"
                value={result.type === 'success' ? 'Passed' : 'Blocked'}
              />
            </View>
          )}
          <View style={styles.resultHeader}>
            <Ionicons
              name={result.type === 'success' ? 'checkmark-circle' : 'shield'}
              size={20}
              color={result.type === 'success' ? '#34C759' : '#FF3B30'}
            />
            <Text
              style={[
                styles.resultTitle,
                result.type === 'blocked' && styles.resultTitleBlocked,
              ]}>
              {result.type === 'success' ? 'Guardrails Passed' : 'Blocked by Guardrail'}
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  sliderRow: {
    ...
    gap: 16, backgroundColor: 'white', borderRadius: 10, padding: 12, marginBottom: 12,
  },
  ...
  charCount: {fontSize: 12, color: '#999', textAlign: 'right', marginTop: 4, marginBottom: 12},
  ...
  resultBlocked: {backgroundColor: '#FFF5F5'},
  badgeRow: {flexDirection: 'row', gap: 8, marginBottom: 12},
  resultHeader: {flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8},
  resultTitle: {fontSize: 15, fontWeight: '600', color: '#34C759'},
  resultTitleBlocked: {color: '#FF3B30'},
