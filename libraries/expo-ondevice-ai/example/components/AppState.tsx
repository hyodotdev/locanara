...
import {initialize, getDeviceCapability} from 'expo-ondevice-ai';
import type {DeviceCapability} from 'expo-ondevice-ai';
...
export function AppStateProvider({children}: {children: ReactNode}) {
  ...
  const initializeSDK = async () => {
    ...
    try {
      ...
      const comingSoonFeatures: FeatureType[] = ['describeImage', 'generateImage'];
      const features = FEATURE_DEFINITIONS.map((def) => {
        const isComingSoon = comingSoonFeatures.includes(def.type);
        return {
          ...def,
          isAvailable: isComingSoon ? false : modelReady && (cap.features?.[def.type as keyof typeof cap.features] ?? false),
          isComingSoon,
        };
      });
