...
export function ChainDemo() {
  ...
  const handleRun = async () => {
    ...
    setIsProcessing(true); setResults([]); setProcessingTime(null);
    ...
    try {
      ...
      if (chainType === 'sequential') {
        ...
        setResults(prev => [...prev, `Step 2 (Classify): ${s2.topClassification.label} (${(s2.topClassification.score*100).toFixed(0)}%)`]);
      } else if (chainType === 'parallel') {
        const [s, c] = await Promise.all([summarize(input), classify(input)]);
        setResults([`Summarize: ${s.summary}`, `Classify: ${c.topClassification.label} (${(c.topClassification.score*100).toFixed(0)}%)`]);
      } else {
        if (input.length > 200) {
          const r = await summarize(input);
          setResults([`Condition: Text is long (${input.length} chars) → Summarize`, `Result: ${r.summary}`]);
        } else {
          const r = await classify(input);
          setResults([`Condition: Text is short (${input.length} chars) → Classify`, `Result: ${r.topClassification.label} (${(r.topClassification.score*100).toFixed(0)}%)`]);
        }
      }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.row}>
        {CHAIN_TYPES.map(ct => (
          <TouchableOpacity key={ct.key} style={[styles.btn, chainType === ct.key && styles.btnSel]} onPress={() => setChainType(ct.key)}>
            <Text style={[styles.btnText, chainType === ct.key && styles.btnTextSel]}>{ct.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
      <TextInput style={styles.input} value={input} onChangeText={setInput} multiline placeholder="Enter text..." placeholderTextColor="#999" />
      <TouchableOpacity style={[styles.runBtn, isProcessing && styles.runBtnDis]} onPress={handleRun} disabled={isProcessing}>
        {isProcessing ? <ActivityIndicator color="white" size="small" /> : <Text style={styles.runBtnText}>Run Chain</Text>}
      </TouchableOpacity>
      {results.length > 0 && (
        <View style={styles.resultCard}>
          {processingTime !== null && <View style={styles.badgeRow}><StatBadge label="Time" value={`${processingTime}ms`} /><StatBadge label="Chain" value={chainType} /></View>}
          {results.map((r, i) => (
            <View key={i} style={[styles.stepRow, i > 0 && styles.stepSep]}><Text style={styles.resultText}>{r}</Text></View>
          ))}
        </View>
      )}
    </ScrollView>
