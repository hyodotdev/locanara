...
export function PipelineDemo() {
  const [input, setInput] = useState(
    'Ths is a mesage with typos and grammer erors.',
  );
  ...
  const handleRun = async () => {
    ...
    try {
      console.log(
        '[DEBUG] pipeline step1 (proofread) request:',
        JSON.stringify({inputLength: input.length}),
      );
      const p = await proofread(input);
      console.log(
        '[DEBUG] pipeline step1 (proofread) response:',
        JSON.stringify(p),
      );
      setSteps((prev) => [
        ...prev,
        {label: 'Step 1: Proofread', result: p.correctedText},
      ]);
      const langLabel = LANGUAGES.find((l) => l.code === targetLang)?.label;
      console.log(
        '[DEBUG] pipeline step2 (translate) request:',
        JSON.stringify({targetLang}),
      );
      const t = await translate(p.correctedText, {targetLanguage: targetLang});
      console.log(
        '[DEBUG] pipeline step2 (translate) response:',
        JSON.stringify(t),
      );
      setSteps((prev) => [
        ...prev,
        {label: `Step 2: Translate → ${langLabel}`, result: t.translatedText},
      ]);
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.row}>
        {LANGUAGES.map((l) => (
          <TouchableOpacity
            key={l.code}
            style={[styles.btn, targetLang === l.code && styles.btnSel]}
            onPress={() => setTargetLang(l.code)}
          >
            <Text
              style={[
                styles.btnText,
                targetLang === l.code && styles.btnTextSel,
              ]}
            >
              {l.label}
            </Text>
