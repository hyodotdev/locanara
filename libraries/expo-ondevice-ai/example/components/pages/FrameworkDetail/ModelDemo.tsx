...
const PRESETS: {key: Preset; label: string; prompt: string}[] = [
  {
    ...
    label: 'Structured',
    prompt:
      'You are a precise assistant. Answer concisely with structured data.',
  },
  {
    key: 'creative',
    label: 'Creative',
...
export function ModelDemo() {
  const [input, setInput] = useState(
    'Explain what on-device AI means in 2 sentences.',
  );
    ...
    try {
      ...
      if (useStreaming) {
        await chatStream(input, {
          ...
          onChunk: (c) => setOutput(c.accumulated),
        });
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.row}>
        {PRESETS.map((p) => (
          <TouchableOpacity
            key={p.key}
            style={[styles.btn, selectedPreset === p.key && styles.btnSel]}
            onPress={() => setSelectedPreset(p.key)}
          >
            <Text
              style={[
                styles.btnText,
                selectedPreset === p.key && styles.btnTextSel,
              ]}
            >
              {p.label}
            </Text>
      ...
      >
        ...
        <View style={[styles.toggle, useStreaming && styles.toggleOn]}>
          <View
            style={[styles.toggleThumb, useStreaming && styles.toggleThumbOn]}
          />
        </View>
      </TouchableOpacity>
      <TextInput
      ...
      />
      <TouchableOpacity
        style={[styles.runBtn, isProcessing && styles.runBtnDisabled]}
        onPress={handleRun}
      ...
      >
        {isProcessing ? (
          ...
          <Text style={styles.runBtnText}>Generate</Text>
        )}
      </TouchableOpacity>
        ...
        <View style={styles.resultCard}>
          {processingTime !== null && (
            <View style={styles.badgeRow}>
              ...
              <StatBadge
                label="Mode"
                value={useStreaming ? 'Stream' : 'Batch'}
              />
            </View>
          )}
