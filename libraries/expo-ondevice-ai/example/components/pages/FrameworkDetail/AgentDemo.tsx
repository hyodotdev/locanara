...
export function AgentDemo() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const relevant = LOCAL_DOCS.filter((d) =>
        keywords.some(
          (kw) =>
            kw.length > 3 &&
      ...
      const s1: ReasoningStep = {
        step: 1,
        thought: `I need to find information about "${query}" in local documents.`,
        action: 'LocalSearchTool',
        observation:
          relevant.length > 0
            ? `Found ${relevant.length} relevant document(s): ${relevant.map((d) => d.title).join(', ')}`
            : 'No matching documents found.',
      };
      setSteps([s1]);

      const context = relevant.map((d) => d.content).join('\n\n');
      const s2: ReasoningStep = {
        step: 2,
        thought:
          relevant.length > 0
            ? 'I have relevant context. Let me answer using this information.'
            : 'No documents found. Let me try general knowledge.',
        action: 'ChatChain',
        observation: '',
      };
      ...
      setSteps((prev) => {
        const u = [...prev];
        u[u.length - 1] = {...u[u.length - 1], observation: result.message};
        return u;
      });

      if (result.message.length > 200) {
        const s3: ReasoningStep = {
          step: 3,
          thought: 'The response is long. Let me summarize.',
        ...
        setSteps((prev) => {
          const u = [...prev];
          u[u.length - 1] = {...u[u.length - 1], observation: sr.summary};
          return u;
        });
