...
이 워크플로는 조사와 영향도 기록만 수행합니다. 사용자가 구현까지
요청하지 않았다면 SDK, 의존성, 버전, 릴리스 상태를 변경하지 않습니다.
...
| `chrome-built-in-ai` | Chrome Built-in AI (Prompt API)         | Web SDK - `packages/web` (`locanara`)        |

### 2. 정보 수집

각 기술은 공식 문서, 공식 릴리스 노트, 해당 프로젝트의 공식 저장소 같은
1차 출처로 확인합니다. 검색 결과 요약이나 블로그만으로 버전/API를 확정하지
말고, 반드시 날짜와 직접 링크를 기록합니다. 가능한 범위에서 병렬로
수집하세요.
...
- `packages/web/src/` - Chrome Built-in AI 관련
...
수집한 정보를 `.claude/knowledge/` 디렉토리에 격리된 조사 스냅샷으로
저장합니다. 이 디렉토리는 기본 agent context에 자동 포함되지 않습니다.
반복 검증을 거쳐 일반 지침으로 승격할 내용만 별도 변경으로
`knowledge/internal/`에 반영합니다.
...
6. **권한 순서**: `AGENTS.md`와 현재 코드/manifest가 연구 스냅샷보다 우선
7. **스냅샷 표시**: "latest"를 영구 사실처럼 쓰지 말고 확인 날짜를 명시
8. **온디바이스 경계**: 클라우드 SDK, 서버 fallback, provider API key를 추천하지 않음
