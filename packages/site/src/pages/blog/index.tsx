...
function Locanara100() {
  return (
    <>
      ...
      <ul>
        <li>
          <strong>iOS/macOS</strong> - Apple Intelligence (iOS 26+) + llama.cpp
          with GGUF models (iOS 17+)
        </li>
        <li>
...
function BlogIndex() {
  ...
  return (
    <div className="doc-page">
      ...
      <p className="text-text-secondary dark:text-text-dark-secondary text-sm mb-8">
        {formatDate(firstPost.date)}
      </p>
      <div>{postContents[firstPost.id]}</div>
    </div>
  );
}
        ...
        <nav className="docs-nav">
          ...
          {years.map((year) => (
            <div key={year} className="mb-4">
              <h4 className="text-xs font-semibold text-text-secondary dark:text-text-dark-secondary uppercase tracking-wider mb-2 px-3">
                {year}
              </h4>
              <ul>
