...
function parseVersions(json: string): Record<string, unknown> {
  try {
    ...
    throw new Error("locanara-versions.json contains invalid JSON.");
  }
}

function ensureVersions(data: Record<string, unknown>): VersionRecord {
  return REQUIRED_KEYS.reduce<Partial<VersionRecord>>((acc, key) => {
...
export const TYPES_RELEASE = Object.freeze({
  ...
  pageUrl: `https://github.com/hyodotdev/locanara/releases/tag/${LOCANARA_VERSIONS.types}`,
  downloadPrefix: `https://github.com/hyodotdev/locanara/releases/download/${LOCANARA_VERSIONS.types}/`,
});
