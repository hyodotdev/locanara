  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-[10000] flex items-start justify-center pt-[15vh]">
        {/* Modal */}
        <div className="w-full max-w-xl mx-4 bg-background-primary dark:bg-background-dark rounded-xl shadow-card border border-primary/10 dark:border-white/10 overflow-hidden">
          {/* Header */}
          <div className="flex items-center gap-3 px-4 border-b border-primary/10 dark:border-white/10">
            ...
            <input
              ...
              className="flex-1 py-3 bg-transparent border-none outline-none text-text-primary dark:text-text-dark-primary placeholder:text-text-secondary dark:placeholder:text-text-dark-secondary text-sm"
            />
            <button
              onClick={onClose}
              className="p-1 rounded hover:bg-primary/5 dark:hover:bg-white/5 text-text-secondary dark:text-text-dark-secondary transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {searchQuery && (
            <div className="max-h-[50vh] overflow-y-auto">
              {filteredApis.length > 0 ? (
                <>
                  <div className="px-4 py-2 text-xs text-text-secondary dark:text-text-dark-secondary border-b border-primary/5 dark:border-white/5">
                      ...
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 transition-colors ${
                          index === selectedIndex
                            ...
                            : "hover:bg-primary/3 dark:hover:bg-white/3"
                        }`}
                        onMouseEnter={() => setSelectedIndex(index)}
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-medium text-text-primary dark:text-text-dark-primary font-mono">
                            {highlightMatch(api.title, searchQuery)}
                          </span>
                          <span className="text-xs px-2 py-0.5 rounded-full bg-primary/5 dark:bg-white/10 text-text-secondary dark:text-text-dark-secondary whitespace-nowrap">
                            {api.category}
                          </span>
                        </div>
                        {api.description && (
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {api.parameters && (
                            <span className="text-xs font-mono text-text-secondary/70 dark:text-text-dark-secondary/70">
                              {highlightMatch(api.parameters, searchQuery)}
                            </span>
                          )}
                          {api.returns && (
                            <span className="text-xs font-mono text-text-secondary/70 dark:text-text-dark-secondary/70">
                              &rarr; {highlightMatch(api.returns, searchQuery)}
                            </span>
                          )}
                        </div>
                      </button>
                ...
                <div className="px-4 py-8 text-center text-sm text-text-secondary dark:text-text-dark-secondary">
          ...
          {/* Footer */}
          <div className="flex items-center gap-4 px-4 py-2.5 border-t border-primary/10 dark:border-white/10 bg-primary/3 dark:bg-white/3">
            <span className="text-xs text-text-secondary dark:text-text-dark-secondary flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-primary/10 dark:bg-white/10 text-[10px] font-mono">
                &uarr;&darr;
              </kbd>{" "}
              Navigate
            </span>
