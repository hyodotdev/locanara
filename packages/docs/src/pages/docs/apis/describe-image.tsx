  ...
  return (
    <div className="doc-page">
      <SEO
        ...
        description="Locanara describeImage API - Generate text descriptions of images using on-device AI."
...
//          with a red ball nearby and trees in the background."`}</CodeBlock>
      ...
      <section>
        <AnchorLink id="example-path" level="h2">
          Example (File Path)
        </AnchorLink>
        <LanguageTabs>
          {{
            swift: (
              <CodeBlock language="swift">{`// Describe image from file path
let result = try await LocanaraClient.shared.executeFeature(
    ExecuteFeatureInput(
        feature: .describeImage,
        input: "",
        parameters: FeatureParametersInput(
            ...
            kotlin: (
              <CodeBlock language="kotlin">{`// Describe image from file path
val result = locanara.executeFeature(
    ExecuteFeatureInput(
        feature = FeatureType.DESCRIBE_IMAGE,
        input = "",
        parameters = FeatureParametersInput(
            ...
            typescript: (
              <CodeBlock language="typescript">{`// Describe image from file path
const result = await Locanara.describeImage({
  input: '',
  parameters: {
    imagePath: '/path/to/image.jpg',
            ...
            dart: (
              <CodeBlock language="dart">{`// Describe image from file path
final result = await Locanara.describeImage(
  input: '',
  parameters: ImageDescriptionParametersInput(
    imagePath: '/path/to/image.jpg',
      ...
      <section>
        <AnchorLink id="use-cases" level="h2">
          Use Cases
        </AnchorLink>

        <h4>Accessibility (Alt Text)</h4>
        <p>Generate descriptive alt text for screen readers:</p>
        <LanguageTabs>
          {{
            swift: (
              <CodeBlock language="swift">{`// Generate alt text for accessibility
let result = try await LocanaraClient.shared.executeFeature(
    ExecuteFeatureInput(
        feature: .describeImage,
        input: "Generate a brief, accessible description",
        parameters: FeatureParametersInput(
            imageDescription: ImageDescriptionParametersInput(
...
// Use for VoiceOver
imageView.accessibilityLabel = result.result?.imageDescription?.description`}</CodeBlock>
            ),
            kotlin: (
              <CodeBlock language="kotlin">{`// Generate alt text for accessibility
val result = locanara.executeFeature(
    ExecuteFeatureInput(
        feature = FeatureType.DESCRIBE_IMAGE,
        input = "Generate a brief, accessible description",
...
// Use for TalkBack
imageView.contentDescription = result.result?.imageDescription?.description`}</CodeBlock>
            ),
            typescript: (
              <CodeBlock language="typescript">{`// Generate alt text for accessibility
const result = await Locanara.describeImage({
  input: 'Generate a brief, accessible description',
...
/>`}</CodeBlock>
            ...
            dart: (
              <CodeBlock language="dart">{`// Generate alt text for accessibility
final result = await Locanara.describeImage(
  input: 'Generate a brief, accessible description',
  parameters: ImageDescriptionParametersInput(imageBase64: imageBase64),
);
...
Semantics(
  ...
  child: Image.file(imageFile),
      ...
      <PageNavigation
        prev={{ to: "/docs/apis/proofread", label: "proofread()" }}
        next={{ to: "/docs/apis/ios", label: "iOS APIs" }}
      />
    </div>
  );
}
