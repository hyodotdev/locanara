...
function ChatAPI() {
  return (
    <div className="doc-page">
      <SEO
        title="Chat API"
        description="Locanara chat API - Conversational AI interactions using on-device models."
        path="/docs/apis/chat"
...
// Output: "The capital of France is Paris."`}</CodeBlock>
      ...
      <section>
        <AnchorLink id="conversation-history" level="h2">
          Conversation with History
        </AnchorLink>
        <p>Pass previous messages for context-aware responses:</p>
        <LanguageTabs>
          {{
            swift: (
              <CodeBlock language="swift">{`// Multi-turn conversation
let result = try await LocanaraClient.shared.executeFeature(
    ExecuteFeatureInput(
        feature: .chat,
        ...
        parameters: FeatureParametersInput(
            chat: ChatParametersInput(
                history: [
                    ChatMessageInput(role: "user", content: "What's the capital of France?"),
                    ChatMessageInput(role: "assistant", content: "The capital of France is Paris.")
                ]
            )
        )
    )
...
if case .chat(let chat) = result.result {
    ...
    // Output: "The capital of Germany is Berlin."
            ...
            kotlin: (
              <CodeBlock language="kotlin">{`// Multi-turn conversation
val result = locanara.executeFeature(
    ExecuteFeatureInput(
        feature = FeatureType.CHAT,
        input = "What about Germany?",
...
val chat = result.result?.chat
println(chat?.message)
// Output: "The capital of Germany is Berlin."`}</CodeBlock>
            ),
            typescript: (
              <CodeBlock language="typescript">{`// Multi-turn conversation
const result = await Locanara.chat({
  input: "What about Germany?",
  parameters: {
    history: [
...
// Output: "The capital of Germany is Berlin."`}</CodeBlock>
            ...
            dart: (
              <CodeBlock language="dart">{`// Multi-turn conversation
final result = await Locanara.chat(
  input: "What about Germany?",
  parameters: ChatParametersInput(
    history: [
...
// Output: "The capital of Germany is Berlin."`}</CodeBlock>
      ...
      <section>
        <AnchorLink id="system-prompt" level="h2">
          System Prompt Example
        </AnchorLink>
        <p>Use system prompts to define AI personality or behavior:</p>
        <LanguageTabs>
          {{
            swift: (
              <CodeBlock language="swift">{`// Chat with system prompt
let result = try await LocanaraClient.shared.executeFeature(
    ExecuteFeatureInput(
        feature: .chat,
        ...
        parameters: FeatureParametersInput(
            chat: ChatParametersInput(
                systemPrompt: "You are a helpful assistant that explains complex topics in simple terms, using analogies a 10-year-old could understand."
            )
        )
    )
)

// Response will be simplified for a younger audience`}</CodeBlock>
            ),
            kotlin: (
              <CodeBlock language="kotlin">{`// Chat with system prompt
val result = locanara.executeFeature(
    ExecuteFeatureInput(
        feature = FeatureType.CHAT,
        input = "Explain quantum computing",
...
// Response will be simplified for a younger audience`}</CodeBlock>
            ...
            typescript: (
              <CodeBlock language="typescript">{`// Chat with system prompt
const result = await Locanara.chat({
  input: "Explain quantum computing",
  parameters: {
    systemPrompt: "You are a helpful assistant that explains complex topics in simple terms, using analogies a 10-year-old could understand.",
...
// Response will be simplified for a younger audience`}</CodeBlock>
            ...
            dart: (
              <CodeBlock language="dart">{`// Chat with system prompt
final result = await Locanara.chat(
  input: "Explain quantum computing",
  parameters: ChatParametersInput(
    systemPrompt: "You are a helpful assistant that explains complex topics in simple terms, using analogies a 10-year-old could understand.",
...
// Response will be simplified for a younger audience`}</CodeBlock>
      ...
      <PageNavigation
        prev={{ to: "/docs/apis/extract", label: "extract()" }}
        next={{ to: "/docs/apis/translate", label: "translate()" }}
      />
    </div>
  );
}
