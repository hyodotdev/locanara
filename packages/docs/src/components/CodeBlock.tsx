...
function CodeBlock({ children, code, language = "graphql" }: CodeBlockProps) {
...
function highlightCode(element: HTMLElement, language: string) {
  ...
  if (language === "typescript" || language === "javascript") {
    ...
    // Ruby syntax highlighting - tokenize first to avoid HTML attribute conflicts
    const lines = text.split("\n");
    const highlightedLines = lines.map((line) => {
      // Check if line is a comment
      if (line.trim().startsWith("#")) {
      ...
      tokens.forEach((token) => {
        if (token.type === "string") {
          ...
          // Keywords
          processed = processed.replace(
            /\b(pod|gem|source|target|do|end|require|include|extend|module|class|def|if|else|elsif|unless|case|when|while|until|for|begin|rescue|ensure|raise|return|yield|self|super|nil|true|false|and|or|not|in)\b/g,
            '<span class="token keyword">$1</span>',
          );
