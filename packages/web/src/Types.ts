...
declare global {
  ...
  interface ChromeRewriter {
    ...
    rewriteStreaming(text: string, options?: { context?: string }): AsyncIterable<string>
    destroy(): void
  }
  ...
  interface LanguageModelConstructor {
    availability(): Promise<'readily' | 'after-download' | 'downloading' | 'unavailable'>
    params(): Promise<{
      defaultTopK: number
      maxTopK: number
    ...
    create(options?: ChromeLanguageModelOptions): Promise<ChromeLanguageModelSession>
  }
  ...
  interface ChromeLanguageModelSession {
    prompt(
      input: string | Array<{ role: string; content: string | Blob | ImageData }>,
    ): Promise<string>
